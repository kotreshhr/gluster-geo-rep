...
if __name__ == '__main__':
    ...
    if blob == None:
        sys.exit(-1)
    try:
        if os.path.lexists(pgfid_bname) and \
           randomgfid != xattr.get(pgfid_bname, "glusterfs.gfid.string", nofollow=True):
            print "GFID MISMATCH: Fixing..Unlinking...: %s" % pgfid_bname
            if os.path.isdir(pgfid_bname): 
                os.rmdir(pgfid_bname)
            else:
                os.unlink(pgfid_bname)
        xattr.set(pargfid, 'glusterfs.gfid.newfile', blob, nofollow=True)
    except:
        ex = sys.exc_info()[1]
